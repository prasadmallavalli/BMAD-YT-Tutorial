...
- ~~No CI pipeline configured — also the only realistic path to eventually verify AC2 (`MainForm` launches without throwing) since the dev machine is macOS.~~ **Resolved 2026-08-11**: `.github/workflows/tests.yml` (windows-latest) added, repo pushed to `github.com/prasadmallavalli/BMAD-YT-Tutorial`, first run green. CI now runs `dotnet test` on the whole solution on every push/PR to `main`; AC2 itself (`MainForm` launches without throwing) still isn't independently verified since no run step invokes the WinForms app, only its tests — that narrower gap remains open.
