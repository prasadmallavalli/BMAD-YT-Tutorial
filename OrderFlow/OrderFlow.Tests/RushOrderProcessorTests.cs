...
public class RushOrderProcessorTests
{
    ...
    [Fact]
    public async Task ConfirmAsync_AppliesTenPercentRushSurchargeOverBaseTotalAndConfirmsOrder()
    {
        ...
        var processor = new RushOrderProcessor(
        ...
        var request = new CreateOrderRequest { CustomerId = 1, OrderType = OrderType.Rush, Items = items };

        // Independently-computed expected value (base total x 1.10), not a mirror of the
        // production expression's own structure. Rounded to currency precision, matching
        // RushOrderProcessor's own rounding — 39.97 x 1.10 = 43.967 has 3 decimal places
        // before rounding, which is exactly the sub-cent-precision bug this rounding guards
        // against.
        ...
        Assert.Equal(OrderType.Rush, result.Value.OrderType);
